---
title: "Docker compose inside docker -- autoscaling multiple containers to zero on Fly.io machines"
publishedAt: "2024-10-21"
summary: "Deploy multi-container applications to production that can scale to zero"
---

Docker Compose has been my trusty sidekick for orchestrating multi-container
applications during development. Its simplicity and effectiveness made me
wonder: could I carry that convenience straight into production? Spoiler
alert—it wasn’t as straightforward as I’d hoped. But the journey led me to some
interesting places, and I think it’s worth sharing for anyone facing similar
hurdles.

## The Initial Plan: EC2 or VPS

My first thought was to deploy the application using an EC2 instance or a
virtual private server. This approach would allow me to run Docker Compose just
as I did locally. However, I was also interested in leveraging features like
horizontal autoscaling and scaling to zero when the application wasn’t in use.
Managing ec2 like this via api or manually is a lot of undifferentiated work.
